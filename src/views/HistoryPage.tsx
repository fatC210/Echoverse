...
import { getStoryTagLabel } from "@/lib/constants/story-tags";
...
const HistoryPage = () => {
  ...
  const exportStoryLabel =
    lang === "zh"
      ? "\u5bfc\u51fa\u6545\u4e8b\u8bb0\u5f55"
      : "Export Story Log";
  const exportWorldLabel =
    lang === "zh"
      ? "\u5bfc\u51fa\u4e16\u754c\u6570\u636e"
      : "Export World Data";
        ...
        {stories.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {stories.map((story, index) => {
              const visibleTags = [
                ...story.tags.preset.map((tag) => getStoryTagLabel(tag, lang)),
                ...story.tags.custom,
              ].slice(0, 6);
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <h3 className="font-serif text-lg font-semibold transition-colors group-hover:text-gradient-primary">
                        {story.title}
                      </h3>
                    ...
                    >
                      {story.status === "completed"
                        ? t("history.status.completed", lang)
                        : t("history.status.playing", lang)}
                    </span>
                  </div>
                  ...
                  <div className="mt-3 flex items-center gap-4 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1">
                      <Clock size={12} />
                      {formatDuration(story.totalDurationSec)}
                    </span>
                    <span className="flex items-center gap-1">
                      <GitBranch size={12} />
                      {story.totalDecisions} {t("history.decisions", lang)}
                    </span>
                  </div>

                  {story.endingName ? (
                    <p className="mt-3 flex items-center gap-1 text-xs text-muted-foreground">
                      <Clapperboard size={12} /> {t("history.ending", lang)}: {story.endingName}
                    </p>
                  ) : null}
                  ...
                  <div className="mt-auto flex items-center justify-between pt-3">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-max min-w-fit">
                        <DropdownMenuItem
                          onClick={(event) => {
                        ...
                        <DropdownMenuItem
                          onClick={(event) => {
                            event.stopPropagation();
                            void handleExportText(story.id);
                          }}
                        >
                          <Download size={14} className="mr-2" /> {exportStoryLabel}
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          onClick={(event) => {
                            event.stopPropagation();
                            void handleExportWorld(story.id);
                          }}
                        >
                          <Package size={14} className="mr-2" /> {exportWorldLabel}
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem
                          onClick={(event) => {
