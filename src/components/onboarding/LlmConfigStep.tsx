  ...
  return (
    <div className="glass-panel p-8 space-y-5">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-2xl font-bold font-serif">{t("onboarding.llm.title", lang)}</h2>
      </div>
      ...
      <div className="space-y-4">
        <div>
          ...
          <Input
            value={llm.baseUrl}
            onChange={(e) => { updateLlm({ baseUrl: e.target.value }); setTestStatus("idle"); }}
            className="bg-secondary border-border mt-1"
          />
          <p className="text-xs text-muted-foreground mt-1">💡 {t("onboarding.llm.baseUrlHint", lang)}</p>
        </div>

        <div>
          <Label className="text-sm text-muted-foreground">{t("onboarding.llm.apiKey", lang)}</Label>
          <div className="relative mt-1">
            <Input
              type={showKey ? "text" : "password"}
              value={llm.apiKey}
              onChange={(e) => { updateLlm({ apiKey: e.target.value }); setTestStatus("idle"); }}
        ...
        <div className="flex items-center gap-3">
          <Button variant="outline" onClick={testConnection} disabled={!llm.apiKey || testStatus === "testing"}>
